...
-- Bounded on purpose: the table of contents stops at the page cap or when the
-- action's time budget is gone, instead of spinning on a broken site.
...
T.view = {
    ...
    cover_referer = SITE .. "/",
    cover_delay_ms = 1600,
    search_hint = "Từ khóa hoặc https://www.tvtruyen.live/ten-truyen.html",
    browse = {
        { text = "Mới đăng", kind = "latest" },
        { text = "Lượt xem", kind = "popular" },
    },
    is_ref = function(text)
        return text:match("^https?://") ~= nil or text:match("^/[%w%-]+%.html$") ~= nil
    end,
}
...
-- Key = the site's /the-loai/<key>.html slug. Adult tags stay hidden until
-- Settings.adultContent() is on (browse() checks again for stale menus).
...
T.genres = {
    ...
    genre("hoc-duong", "Học Đường"),
    genre("thanh-xuan", "Thanh Xuân"),
    ...
    genre("sac-hiep", "Sắc Hiệp (18+)", true),
    genre("adult", "Adult (18+)", true),
    genre("mature", "Mature (18+)", true),
    genre("ecchi", "Ecchi (18+)", true),
    genre("incest", "Incest (18+)", true),
    genre("netorare", "Netorare (18+)", true),
}
...
function T.browse(kind, page)
    ...
    if selected then
        ...
        return list("/the-loai/" .. selected.key .. ".html", page)
    end
...
function T.getSeries(ref)
    ...
    while true do
        ...
        if next_page > MAX_TOC_PAGES or Http.expired() then
