...
local function showSeries(ref) Novels.showSeries(ref, Wattpad) end

function UI.promptSearch()
    Catalog.promptText{ title = _("Tìm truyện hoặc nhập URL Wattpad"),
        hint = _("Từ khóa hoặc https://www.wattpad.com/story/…"),
        on_submit = function(text)
            text = (text or ""):match("^%s*(.-)%s*$")
            if text == "" then return end
            if text:match("^https?://") or text:match("^/story/") or text:match("^%d+$") then
                showSeries(text)
            else
                UI.list(nil, text)
            end
    ...
    Network.whenOnline(function()
        ...
        Trapper:wrap(function()
            local ok, result, err = pcall(function()
                Trapper:info(_("Đang tải Wattpad…"))
                if query then return Wattpad.search(query, page) end
                return Wattpad.browse(kind, page)
            end)
            busy = false
            Trapper:clear()
            if not ok or not result then
                UIManager:show(InfoMessage:new{ text = ok and err or _("Không đọc được Wattpad. Thử nhập URL truyện.") })
                return
            end
            UIManager:nextTick(function()
                ...
                local payload = { title = query and ("Wattpad — " .. query) or "Wattpad",
                    items = items, offset = offset, site_page = page, has_more = result.has_more,
                    source_id = "wattpad", base_url = "https://www.wattpad.com",
                    cover_referer = "https://www.wattpad.com/", cover_delay_ms = 1600,
...
function UI.openSource()
    Catalog.show{ title = "Wattpad", on_search = UI.promptSearch, items = {
        { text = _("Nổi bật"), callback = function() UI.list("hot") end },
        { text = _("Đề cử"), callback = function() UI.list("featured") end },
        { text = _("Mới"), callback = function() UI.list("new") end },
    } }
end
