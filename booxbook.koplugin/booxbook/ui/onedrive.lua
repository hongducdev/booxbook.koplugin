...
function UI.showLogin(info)
    info = info or OneDrive.loginInfo()
    local items = {}
    if info then
        ...
        items[#items + 1] = { text = _("Đã đăng nhập, kiểm tra"), keep_menu_open = true, callback = function()
            online(_("Đang kiểm tra đăng nhập OneDrive..."), OneDrive.finishLogin, function()
                notify(_("Đã đăng nhập OneDrive."))
                Catalog.clearStack()
                UI.showFolder(nil, "OneDrive")
            end)
        end }
    end
    items[#items + 1] = { text = info and _("Tạo mã đăng nhập mới") or _("Đăng nhập OneDrive"),
        keep_menu_open = true, callback = function()
            online(_("Đang tạo mã đăng nhập OneDrive..."), OneDrive.startLogin, UI.showLogin)
        end }
    Catalog.show{ title = _("Đăng nhập OneDrive"), items = items }
end

function UI.download(item)
    Catalog.confirm(_("Tải về thư viện?") .. "\n" .. item.name .. " · " .. sizeText(item.size), function()
        online(_("Đang tải:") .. " " .. item.name, function() return OneDrive.download(item) end, function(path)
...
function UI.open()
    if OneDrive.clientId() == "" then
        notify(_("Chưa cấu hình Microsoft client ID. Vào Cài đặt → OneDrive."))
        return
    end
    local info = OneDrive.loginInfo()
    if info or not OneDrive.hasAuth() then UI.showLogin(info); return end
    UI.showFolder(nil, "OneDrive")
end
