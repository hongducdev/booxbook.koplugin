...
local function showSeries(ref) Novels.showSeries(ref, MeTruyenCV) end

function UI.promptSearch()
    Catalog.promptText{ title = _("Tìm truyện hoặc nhập URL MeTruyenCV"),
        hint = _("Từ khóa hoặc https://metruyencv.com/truyen/…"),
        on_submit = function(text)
            text = (text or ""):match("^%s*(.-)%s*$")
            if text == "" then return end
            if text:match("^https?://") or text:match("^/truyen/") or text:match("^%d+$") then
                showSeries(text)
            else
                UI.list(nil, text)
            end
    ...
    Network.whenOnline(function()
        ...
        Trapper:wrap(function()
            local ok, result, err = pcall(function()
                Trapper:info(_("Đang tải MeTruyenCV…"))
                if query then return MeTruyenCV.search(query, page) end
                return MeTruyenCV.browse(kind, page)
            end)
            busy = false
            Trapper:clear()
            if not ok or not result then
                UIManager:show(InfoMessage:new{ text = ok and err or _("Không đọc được MeTruyenCV. Thử nhập URL truyện.") })
                return
            end
            UIManager:nextTick(function()
                ...
                local payload = { title = query and ("MeTruyenCV — " .. query) or "MeTruyenCV",
                    items = items, offset = offset, site_page = page, has_more = result.has_more,
                    source_id = "metruyencv", base_url = "https://metruyencv.com",
                    cover_referer = "https://metruyencv.com/", cover_delay_ms = 1600,
...
function UI.openSource()
    Catalog.show{ title = "MeTruyenCV", on_search = UI.promptSearch, items = {
