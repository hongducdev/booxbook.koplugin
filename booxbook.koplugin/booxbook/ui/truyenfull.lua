...
local function showSeries(ref) Novels.showSeries(ref, TruyenFull) end

function UI.promptSearch()
    Catalog.promptText{ title = _("Tìm truyện hoặc nhập URL Truyện Full"),
        hint = _("Từ khóa hoặc https://truyenfull.live/ten-truyen/"),
        on_submit = function(text)
            text = (text or ""):match("^%s*(.-)%s*$")
            if text == "" then return end
            if text:match("^https?://") or text:match("^/[%w%-]+/?$") then
                showSeries(text)
            else
                UI.list(nil, text)
            end
    ...
    Network.whenOnline(function()
        ...
        Trapper:wrap(function()
            local ok, result, err = pcall(function()
                Trapper:info(_("Đang tải Truyện Full…"))
                if query then return TruyenFull.search(query, page) end
                return TruyenFull.browse(kind, page)
            end)
            busy = false
            Trapper:clear()
            if not ok or not result then
                UIManager:show(InfoMessage:new{ text = ok and err or _("Không đọc được Truyện Full. Thử nhập URL truyện.") })
                return
            end
