...
W.browse = function(kind, page)
    assert(kind == 'hot'); page_seen = page
    requests = requests + 1
    local items = {}
    for i = 1, 8 do items[i] = { title = 'Public', ref = tostring(i), cover = 'https://img.wattpad.com/cover.jpg' } end
    return { items = items, has_more = page == 1 }
end
W.search = function(query) assert(query == 'test'); return nil, 'API changed' end
local UI = dofile('booxbook.koplugin/booxbook/ui/wattpad.lua')
...
assert(grid.source_id == 'wattpad' and grid.items[1].cover and not grid.cover_cookies)
...
UI.openSource(); shown.on_search(); prompt.on_submit(' https://www.wattpad.com/story/1 ')
assert(selected == 'https://www.wattpad.com/story/1')
prompt.on_submit('  '); assert(selected == 'https://www.wattpad.com/story/1')
prompt.on_submit('/story/2'); assert(selected == '/story/2')
prompt.on_submit('3'); assert(selected == '3')
UI.list('hot'); assert(page_seen == 1, 'error must release busy guard')
...
print('Wattpad menu, pagination, URL fallback and error recovery checks passed')
