...
local W = require('booxbook.sources.tvtruyen')
...
W.browse = function(kind, page)
    ...
    requests = requests + 1
    local items = {}
    for i = 1, 8 do items[i] = { title = 'Public', ref = tostring(i), cover = 'https://img.tvtruyen.com/cover.jpg' } end
    return { items = items, has_more = page == 1 }
end
W.search = function(query) assert(query == 'test'); return nil, 'API changed' end
local UI = dofile('booxbook.koplugin/booxbook/ui/tvtruyen.lua')
...
assert(grid.source_id == 'tvtruyen' and grid.items[1].cover and not grid.cover_cookies)
...
UI.openSource(); shown.on_search(); prompt.on_submit(' https://www.tvtruyen.live/dao-quan.html ')
assert(selected == 'https://www.tvtruyen.live/dao-quan.html')
prompt.on_submit('  '); assert(selected == 'https://www.tvtruyen.live/dao-quan.html')
prompt.on_submit('/dao-quan.html'); assert(selected == '/dao-quan.html')
...
assert(grid.title == 'TVTruyen — tien-hiep', 'genre name labels the grid')
W.browse, W.search = browse, search
for _, name in ipairs(names) do package.loaded[name] = saved[name] end
print('TVTruyen menu, pagination, URL fallback and error recovery checks passed')
